...
[~,~,ext] = fileparts(wavfilename);
if strcmp(ext,'.wav')
    % to extract bits per sample
    [f_audio,fs] = audioread(strcat(dirAbs,dirRel,wavfilename),'native');
    nbits = getbits(f_audio);
    f_audio = audioread( strcat(dirAbs,dirRel,wavfilename) );
else
